...
class Nocode_test:
    ...
    def Make_test(self,path=False,string=False):
        ...
        if string:
            self.make_xml(json_to_dect(json.loads(string)))
            self.make_dict_to_table(json.loads(string))
        ...
        if path:
            orginal = get_stack(path=path)
            stack = get_stack(path=path)[1:]
        else:
            orginal = get_stack(string=string)
            stack = get_stack(string=string)[1:]
